...
async function postNui(eventName, payload = {}) {
    try {
        ...
        if (!response.ok) {
            console.error(`[lsrp_policevehicleparking] NUI callback failed: ${eventName} (${response.status})`);
        }
    } catch (error) {
        console.error(`[lsrp_policevehicleparking] Failed to post NUI callback: ${eventName}`, error);
    }
}
